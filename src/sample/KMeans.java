...
/**
 * Created by gardi on 14.09.2017.
 */
public class KMeans {
    ...
    public KMeans(int[][] properties) {
        ...
        for(int i = 0; i < TOTAL_CLUSTERS; i++) {
            Centroid c = new Centroid(ImageProcessor.TOTAL_PROPERTIES);
            for (int j = 0; j < ImageProcessor.TOTAL_PROPERTIES; j++) {
    ...
    public ArrayList<Data> cluster() {
        ...
        while(dataList.size() < properties.length) {
            Data data = new Data(ImageProcessor.TOTAL_PROPERTIES);
            for(int i = 0; i < ImageProcessor.TOTAL_PROPERTIES; i++) {
            ...
            for(int i = 0; i < TOTAL_CLUSTERS; i++) {
                int[] total = new int[ImageProcessor.TOTAL_PROPERTIES];
                int inCluster = 0;

                for(int j = 0; j < dataList.size(); j++) {
                    if(dataList.get(j).getCluster() == i) {
                        for(int k = 0; k < ImageProcessor.TOTAL_PROPERTIES; k++) {
